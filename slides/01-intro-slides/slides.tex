...
\begin{frame}
  ...
  %     \begin{tabular}{c}
  %       \textsc{Learn}(\includegraphics[height=1in]{mnist-digits}) $\rightarrow g$, 
  %       \textsc{Learn}(\includegraphics[height=1in]{fashion-mnist-sprite-some}) $\rightarrow h$
  % \end{tabular}
\end{center}
%\begin{itemize}
  % \item Then we would expect
  %   $g(\includegraphics[height=1cm]{mnist-0})=0$,
  %   $g(\includegraphics[height=1cm]{mnist-1})=1$
  % \item $h(\includegraphics[height=1cm]{fashion-mnist-boot})=\text{shoe/9}$,
  % $h(\includegraphics[height=1cm]{fashion-mnist-pants})=\text{pants/1}$
  %\end{itemize}
  \textbf{Learn} is a learning algorithm you have coded, which outputs
    $g$ and $h$. Q: what happens if you do
