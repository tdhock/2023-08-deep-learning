...
max_deg = 360
N_sim =100
degrees=np.random.uniform(0,max_deg,N_sim)
...
label = true+ np.random.normal(scale=0.1,size=N_sim)
missing=20
set_name = np.where(
...
train_df.set_name = np.tile(["subtrain","validation"],int(train_nrow/2))
import torch
...
feature_dict = {
    ...
    "degrees01":(["degrees01"],0.001),
    "sin,cos":(["sin","cos"],0.004)
    }
def tensor(x):
